...
（※このエフェクトと`key`実装は項目13で刷新・修正済み。詳細は項目13を参照。）

### 13. 斬撃エフェクトを刷新し、撃破の瞬間（キリングブロー）でも見えるように修正

**内容:** 3つの変更をまとめて実施。

1. **斬撃エフェクトの刷新:** 絵文字を⚔️から🗡️に変更し、モーションを「その場での拡大・回転」から「右上から左下へ斬り抜ける」実際の斜めスイープに変更（`src/index.css`の`slash`keyframeを再定義）。
2. **撃破の瞬間（キリングブロー）のリンガー追加:** それまでは敵を倒した瞬間に即座にDefeated/Result画面へ遷移していたため、とどめの一撃では斬撃エフェクトが表示される前に画面が切り替わってしまっていた。`App.tsx`に500msの「リンガー」を実装し、キリングブロー後もBattleScreenを一定時間表示し続けてから遷移するように変更。`reducer.ts`には一切手を加えず、`App.tsx`側の`useEffect`とタイマーのみで完結させている。
3. **再マウント漏れによる無反応バグの修正:** 上記のリンガー実装後もレビューで、キリングブロー時は斬撃・フィードバックのアニメーションが実際には再生されないバグが発覚。原因は、アニメーション再スタート用の`key`に`question.id`を使っていたが、キリングブロー時は`reducer.ts`が`question: null`を返すため、`App.tsx`はリンガー中`lastQuestionRef.current`（＝1つ前の描画で使われていたのと同一の`Question`オブジェクト）にフォールバックしており、`question.id`が変化せずReactが要素を再マウントしない、というものだった（ボス撃破時の「せいかい！」フィードバックも同様の理由で発生していた）。`state.questionsAnswered`（`ANSWER`アクションのたびに正誤・撃破を問わず必ずインクリメントされるカウンター）を`answerSeq`という新しいpropとして`BattleScreen`に渡し、`key`をこれに切り替えることで解決。

**関連ファイル:** `src/index.css`, `src/screens/BattleScreen.tsx`, `src/app/App.tsx`。
